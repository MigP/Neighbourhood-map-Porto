...
// Places view model
		...
		places: [
				{
					...
					"alt_names": ["Miguel Bombarda - Art District"],
					"tags": ", Porto",
					"type": "museum",
					"location": {
						"lat": 41.149677,
	...
	placesViewModel.sortPlaces = (function sortPlaces(places) { // Sorts places alphabetically by name
		places.sort(function(a, b){
		    var nameA = removeDiacritics(a.name.toLowerCase()), nameB = removeDiacritics(b.name.toLowerCase());
		    if (nameA < nameB) {
		        return -1;
		    } else if (nameA > nameB) {
		        return 1;
    ...
    placesViewModel.search = ko.computed(function() { // Filters places depending on type filters and on search input
        ...
        var searchResult = removeDiacritics(this.searchBox().toLowerCase());
        ...
        return ko.utils.arrayFilter(placesViewModel.places, function(place) {
        	...
        	for (var i = 0; i < place.alt_names.length + 1; i++) {
        		if (i == 0) {
					var title = removeDiacritics(place.name.toLowerCase()); // Compares search input with iterated place name
        		} else {
	            	var title = removeDiacritics(place.alt_names[i-1].toLowerCase()); // Compares search with its alternative names
        		}
